...
export const metadata: Metadata = {
  ...
  description:
    "Fahrzeug online zulassen: Mit dem vorläufigen Zulassungsnachweis aus dem i-Kfz-Verfahren dürfen Sie bis zu 14 Tage fahren, während Papiere und Plaketten per Post kommen.",
...
const ablauf = [
  ...
  },
  {
    ...
    text: "Der vorläufige Zulassungsnachweis steht sofort bereit. Ausdrucken, von außen gut lesbar ins Fahrzeug legen, Schilder montieren, Bescheid mitführen – fertig.",
  },
  {
...
export default function Page() {
  return (
    <>
      <div className="relative overflow-hidden border-b border-line bg-card">
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 bg-[radial-gradient(55rem_28rem_at_80%_-15%,color-mix(in srgb, var(--color-accent) 14%, transparent),transparent)]"
        />
        <Container className="relative grid gap-12 py-14 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:py-20">
          <div>
            <p className="inline-flex items-center gap-2 rounded-full bg-ok-veil px-3 py-1.5 text-xs font-semibold text-ok">
            ...
            <h1 className="mt-6 text-balance text-4xl font-semibold leading-[1.1] tracking-tight text-ink sm:text-5xl">
              Zulassen und losfahren,{" "}
              <span className="text-accent-bright">bevor die Papiere kommen</span>
            </h1>

            <Lead>
              Die internetbasierte Fahrzeugzulassung erlaubt die sofortige
              Inbetriebsetzung: Nach dem Bescheid dürfen Sie{" "}
              {NACHWEIS_GUELTIGKEIT_TAGE} Tage fahren, während
              Zulassungsbescheinigung und Plaketten noch unterwegs sind. Die
              Frist beginnt mit dem Abruf der Entscheidung – nicht mit der
              Bestellung.
            </Lead>
          ...
          <Card className="bg-card">
            ...
            <ul className="mt-5 space-y-3 text-sm text-ink-2">
              {[
                `Fahrberechtigung für bis zu ${NACHWEIS_GUELTIGKEIT_TAGE} Tage ab Abruf der Entscheidung (§ 31 FZV)`,
            ...
            <p className="mt-6 rounded-lg bg-card p-4 text-xs leading-relaxed text-ink-2">
      ...
      <Section tone="raised">
        ...
        <ol className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {ablauf.map((s, i) => (
            <li
              key={s.titel}
              className="relative rounded-[var(--radius-card)] border border-line bg-card p-6"
            >
              <span className="absolute -top-3 left-6 grid h-8 w-8 place-items-center rounded-full bg-accent text-sm font-bold text-[var(--color-on-accent)]">
                {i + 1}
              </span>
              <h3 className="mt-4 text-base font-semibold text-ink">{s.titel}</h3>
      ...
      <Section tone="base" id="voraussetzungen">
        <div className="grid gap-12 lg:grid-cols-2">
          <div>
            ...
            <ul className="mt-6 space-y-3">
              {[
                ...
                <li key={titel} className="rounded-lg border border-line bg-card p-5">
          ...
          <div>
            ...
            <p className="mt-6 rounded-lg bg-accent-veil p-5 text-sm leading-relaxed text-accent-bright">
