        ...
        <nav aria-label="Übersicht" className="mb-10 flex flex-wrap gap-2">
          {guides.map((g) => (
            <a
              key={g.id}
              href={`#${g.id}`}
              className="rounded-full border border-line bg-white px-4 py-2 text-sm font-medium text-ink-700 hover:border-brand-500 hover:text-brand-800"
            >
              {g.title}
            </a>
          ))}
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          {guides.map((g) => (
            <Card key={g.id} className="scroll-mt-28" >
              <h2 id={g.id} className="scroll-mt-28 text-xl font-semibold text-ink-900">
                {g.title}
              </h2>
              <p className="mt-2 text-sm leading-relaxed text-ink-500">{g.intro}</p>
