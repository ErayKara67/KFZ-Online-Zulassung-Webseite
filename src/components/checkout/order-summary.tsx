    ...
    >
      ...
      <dl className="mt-5 space-y-2.5 text-sm">
        {lines.map((line, i) => (
          <div key={`${line.label}-${i}`} className="flex justify-between gap-4">
            ...
            <dd className="font-medium text-ink-900">{formatPrice(line.amount)}</dd>
          </div>
        ))}
      </dl>
      ...
      <div className="mt-5 border-t border-line pt-4">
        <div className="flex items-baseline justify-between">
          <span className="text-sm font-semibold text-ink-900">Gesamt</span>
          <span className="text-2xl font-semibold text-ink-900">{formatPrice(total)}</span>
        </div>
      ...
      {service ? (
        <p className="mt-5 rounded-lg bg-white p-4 text-xs leading-relaxed text-ink-500">
          <span className="font-semibold text-ink-700">Bearbeitungszeit:</span>{" "}
          {service.duration}
        </p>
      ) : null}
    </aside>
