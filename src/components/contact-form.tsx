...
export function ContactForm() {
  ...
  if (state === "sent") {
    return (
      <div className="rounded-[var(--radius-card)] border border-ok-700/25 bg-ok-100 p-8">
        <h2 className="text-lg font-semibold text-ok-700">Nachricht ist angekommen</h2>
        <p className="mt-2 text-sm text-ok-700">
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-[var(--radius-card)] border border-line p-6 sm:p-8" noValidate>
      ...
      <label htmlFor="k-privacy" className="mt-6 flex cursor-pointer items-start gap-3 text-sm text-ink-700">
        <input
          id="k-privacy"
          name="privacy"
          type="checkbox"
      ...
      {error ? (
        <p role="alert" className="mt-5 rounded-lg bg-bad-100 px-4 py-3 text-sm font-medium text-bad-700">
          {error}
        </p>
      ) : null}
